...
def main(args):
    ...
    print(clean_df)
    clean_df, indexed_txt_list, vocab_dict, vocab_count = util.preprocess_and_index(clean_df, ngram=1)
    topic_vec = clean_df["variety"]

    min = 500

    for i in range(len(indexed_txt_list)):
        if len(indexed_txt_list[i]) < min:
            min = 500

    data = torch.tensor([data[:min] for data in indexed_txt_list])
    ...
    for step in range(args.num_steps):
        ...
        if step % 10 == 0:
            ...
            # tr = poutine.trace(guide).get_trace(data, args)
