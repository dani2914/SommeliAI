...
def main(neural_args):
    ...
    USE_CUDA = False
    num_topic = 10
    RUN_NAME = "sLDA_" + str(TESTING_SUBSIZE) + "_" +str(SUBSAMPLE_SIZE) + "_" + str(num_topic) + "_enumerated"   # 1/k 1/k error 1/v 1/k error
    print(RUN_NAME)
    jitter = 1e-8
    # # #
    if USE_CUDA:
    ...
    for step in range(num_step):
        ...
        if step % 100 == 0:
            ...
            for ix in range(len(alpha_ix)):
                phi_p = phi[ix]
                        # zs = []
                        # for j in range(len(indexed_txt_list[alpha_ix[ix]])):
                        #     z_assignment = np.random.choice(np.arange(num_topic), 1,
                        #                                 p=phi_p[j])
                        #     zs.append(z_assignment)
