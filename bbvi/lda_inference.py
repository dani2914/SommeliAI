...
def dirichlet_expectation(alpha):
    ...
    return(psi(alpha) - psi(np.sum(alpha, 1))[:, n.newaxis])
...
def lda_inference(doc, lda, adagrad=False):
    ...
    while (converged > 1e-3 and var_ite < 1e3):
        ...
        for n in range(doc.length):
            ...
            ln_p_w = lda.log_prob_w[which_j][:, doc.words[n]] # S length vector
            ...
            # print (lda.log_prob_w[which_j][:, doc.words[n]])
...
def compute_likelihood(doc, lda, phi, var_gamma):
    ...
    likelihood = lgamma(lda.alpha * lda.num_topics) \
                - lda.num_topics * lgamma(lda.alpha) \
                - (lgamma(var_gamma_sum))

    for k in range(lda.num_topics):

        likelihood += (lda.alpha - 1)*(dig[k] - digsum) \
...
def main():
    ...
    for i, e in enumerate(docset):
        # i = i + 41
        print(i)
        likelihood = lda_inference(corpus[e], model, False)
        if (i % 10) == 0:
